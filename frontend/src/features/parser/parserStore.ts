...
// NETWORK_ERROR_MESSAGE precedent in types/api.ts. Backend errors surface the
// API's own bilingual message via ApiError.
 ...
 * which result CARDS are displayed — video mode shows `type === 'video'`,
 * music mode shows `type === 'music'`. The parse request itself is
 * mode-agnostic (the backend resolves whatever the URLs point to), so the
 * filter is pure presentation and switching modes never requires a re-parse.
 * Cards of other types (e.g. `image`) are hidden in both modes for now.
 */
export function selectVisibleResults(results: ParseResult[], mediaMode: MediaMode): ParseResult[] {
  return results.filter((result) => result.type === mediaMode);
}
