"""Tests for the legacy parser adapter (Task 10): the parse-video-py
integration extracted from the engine adapter. All engine calls are faked
(monkeypatched parse_video_share_url, httpx.MockTransport for the size
probe) — the suite never touches the network."""
...
import app.adapters.parser_legacy as parser_legacy
...
from app.adapters.parser_legacy import LegacyParserAdapter
...
class TestRouting:
    def test_video_platform_routes_to_parse_video_py(self, monkeypatch):
        ...
        monkeypatch.setattr(parser_legacy, "parse_video_share_url", fake_parse)
    ...
    def test_music_hosts_are_not_legacy_routes(self):
        for url in ("https://music.163.com/#/song?id=1", "https://y.qq.com/n/ryqq/songDetail/001x"):
            assert parser_legacy._route(url) is None
    ...
    def test_route_table_covers_engine_mapping(self):
        ...
        video_hosts = {host for host, _ in parser_legacy.LEGACY_ROUTES}
...
class TestFileSizeProbe:
    ...
    def test_probe_failure_leaves_size_none(self, monkeypatch):
        ...
        monkeypatch.setattr(parser_legacy, "parse_video_share_url", fake_parse)
        adapter = LegacyParserAdapter(transport=httpx.MockTransport(handler))
        result = adapter.parse(ParseCommand(urls=["https://v.douyin.com/abc/"]))[0]
...
class TestEngineErrorTranslation:
    def test_connect_error_becomes_engine_network_error(self, monkeypatch):
        ...
        with pytest.raises(EngineNetworkError):
            LegacyParserAdapter().parse(ParseCommand(urls=["https://v.douyin.com/abc/"]))
    ...
    def test_slow_engine_call_hits_configured_timeout(self, monkeypatch):
        ...
        monkeypatch.setattr(parser_legacy, "parse_video_share_url", slow_parse)
        with pytest.raises(EngineTimeoutError):
            LegacyParserAdapter(timeout_seconds=0.01).parse(
                ParseCommand(urls=["https://v.douyin.com/abc/"])
            )
