...
| 12 | Python dependency audit | `python -m pip_audit` (after `python backend/scripts/install_backend_dependencies.py`) | No known vulnerabilities; only the documented fixed-SHA Git `parse-video-py` skip; no broad ignores | ⏳ run on the release environment |
